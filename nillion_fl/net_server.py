            ...
            try:
                for request in request_iterator:
                    ...
                    if self.is_initial_request(request):
                        ...
                        logger.info(f"[SERVER][{stream_id}] Received store ids: {request.store_ids}")
                        with self.lock:
                            if self.learning_in_progress:
                                ...
                                logger.warning(f"[SERVER][{stream_id}] Received store ids when learning is not in progress")
            except grpc.RpcError as e:
                logger.error(f"[SERVER][{stream_id}] RPC Error: {e}")
            finally:
                logger.error(f"[SERVER][{stream_id}] Handling client request thread stopped")
            ...
            while stream_id in self.clients_queue:
                if not self.clients_queue[stream_id].empty():
                    ...
                    if not self.learning_complete.wait():  # 60 seconds timeout
                        logger.warning(f"[SERVER][{stream_id}] Learning iteration timed out")
