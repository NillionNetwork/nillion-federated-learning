...
from nillion_fl.server import FederatedLearningServicer  # Replace 'your_module' with the actual module name

class TestFederatedLearningServicer(unittest.TestCase):
    def setUp(self):
        self.servicer = FederatedLearningServicer(num_parties=2)
        self.test_server = grpc_testing.server_from_dictionary(
            {fl_pb2.DESCRIPTOR.services_by_name['FederatedLearningService']: self.servicer},
    ...
    def test_schedule_learning(self):
        ...
        rpc_a = self.test_server.invoke_stream_stream(
            fl_pb2.DESCRIPTOR.services_by_name['FederatedLearningService'].methods_by_name['ScheduleLearningIteration'],
            (), 
            None
        )
        ...
        self.servicer.handle_client_disconnect(server_response_1.program_id)
